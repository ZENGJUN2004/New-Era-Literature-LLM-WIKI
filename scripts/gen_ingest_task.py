...
def load_wiki_source_ids(root="wiki"):
    """扫描 wiki/ 下所有 md 的 sources 引用。

    返回 (已用 raw 文档 ID 集合, 原文 URL id 集合)。
    页面可能直接以原文 URL 引用（c<栏目>-<id>.html 形式），
    已验证全部 raw 文档的 contentId 以该 URL id 结尾（contentId = "10000" + url id）。
    """
    used = set()
    url_ids = set()
    ...
    for dirpath, _, files in os.walk(root):
        for fn in files:
            ...
            for m in re.finditer(r"c\d+-(\d{7,})\.html", content):
...
def gen_task_file(raw_dir="raw/markdown", tasks_dir="tasks", wiki_root="wiki"):
    ...
    used, url_ids = load_wiki_source_ids(wiki_root)

    def is_used(cid: str) -> bool:
        return cid in used or any(cid.endswith(u) for u in url_ids)
    ...
    for year in sorted(os.listdir(raw_dir)):
        ...
        for fn in sorted(os.listdir(ydir)):
            if fn.endswith(".md") and not is_used(extract_doc_id(fn)):
